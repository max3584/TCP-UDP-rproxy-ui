...
export interface RoleConfig {
  ...
  claim: string;
  adminRole: string;
  // 空なら、サインインできる人はだれでも user（ロールを使わない運用）
  userRole: string;
...
// 環境変数から読む。未設定なら既定（realm_access.roles、rproxy-admin、rproxy-user、ポートの制限なし）
export function roleConfig(env: Record<string, string | undefined> = process.env): RoleConfig {
  ...
  return {
    ...
    userRole: env.RPROXY_UI_USER_ROLE === undefined ? 'rproxy-user' : env.RPROXY_UI_USER_ROLE.trim(),
